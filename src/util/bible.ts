...
const regexes = [
  // Only the book (not an alias) (maybe this should only be when the book name is preceded by "Book of"?)
...
export function* getBibleReferences(text: string) {
  ...
  while (lastIndex <= text.length) {
    ...
    const sortedRes = orderBy(
      res,
      [(arr) => arr[1]?.index, (arr) => arr[0].lastIndex],
      ['asc', 'desc'],
    );
    ...
    const { book, verseSuffix, verseEndSuffix } = groups as Record<
      string,
      string | undefined
    >;
    let { chapter, verse, verseEnd, chapterEnd } = groups as Record<
      string,
      string | undefined
    >;
