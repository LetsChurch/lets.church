...
// The general cache is intentionally fail-fast, but the abuse limiter benefits
// from a very short startup grace period so the first request after boot lands
// in the shared bucket instead of immediately falling back to process memory.
 ...
 * disabled or unavailable so callers can apply a process-local fallback.
 */
