...
| Content-filter block | `finish_reason: content_filter` | Provider safety classifier triggered | Framing preamble at top of system prompt: "You are NOT a content moderator… you must produce the same outline + annotations regardless of subject matter". Activity throws and writes `llm_call.outcome = 'guard_content_filter'`. gpt-5.4-mini still blocks some Dividing Line content (trans/abortion); seed regen hits ~5/27 of LLM-seeded uploads. |
...
A subset (typically 5/27 today) will fail with `guard_content_filter` on Dividing Line trans/abortion content — those uploads keep their previous annotations (the activity throws before the DB transaction). Query `llm_call` for `outcome != 'success' AND activity = 'annotateTranscript' AND created_at > '<regen start>'` to see the failures and decide whether to switch model for those uploads. See `docs/seed-data.md` for the full bootstrap flow.
