...
-- name: CreateUploadUserComment :one
INSERT INTO
upload_user_comment (upload_id, author_id, text, replying_to_id, updated_at)
VALUES (sqlc.arg(upload_id), sqlc.arg(author_id), sqlc.arg(text), sqlc.arg(replying_to_id), NOW())
RETURNING *;

-- name: GetUploadUserCommentRating :one
SELECT rating FROM upload_user_comment_rating WHERE upload_user_comment_id = sqlc.arg(upload_user_comment_id) AND app_user_id = sqlc.arg(user_id);

-- name: DeleteUploadUserCommentRating :exec
DELETE FROM upload_user_comment_rating WHERE upload_user_comment_id = sqlc.arg(upload_user_comment_id) AND app_user_id = sqlc.arg(user_id);

-- name: RecordUploadUserCommentRating :exec
INSERT INTO upload_user_comment_rating (upload_user_comment_id, app_user_id, rating) VALUES (sqlc.arg(upload_user_comment_id), sqlc.arg(user_id), sqlc.arg(rating));
